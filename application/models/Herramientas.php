...
class Herramientas extends  CI_Model{
    ...
    function generarCodigoCerrarSesion(){
        $longitud = 6;
        $key = '';
    ...
    function notificarIngreso($idUsuario,$nombres,$apellidos,$correoElectronico,$ipAntigua, $ipNueva){
        ...
        $info= $this->detect();
        $codigoCerrarSesion = $this->generarCodigoCerrarSesion();
                                            ...
                                            <td>
                                                ...
                                                <div style="font-family:Roboto-Regular,Helvetica,Arial,sans-serif;font-size:13px;color:#202020;line-height:1.6;padding-left:16px;padding-right:16px;padding-bottom:32px;padding-top:24px">
                                                    <div class="m_-1758319509973513776v2sp">Alguien acaba de acceder a tu cuenta de practict desde un nuevo dispositivo. Te enviamos este correo electrónico para asegurarnos de que hayas sido tú.
                                                     ...
                                                     <br>Sistema operativo: '.$info["os"].'
                                                     <br>Navegador '.$info["browser"].'
                                                                                                       
                                                        <div style="padding-top:24px" align="center">
															<form action="http://practict.simpleteamgroup.com/practict/controlCerrarSesionCorreo" method="POST" role="form">
															<input type="hidden" name="idUsuario" value="'.$idUsuario.'">
															<input type="hidden" name="correoElectronico" value="'.$correoElectronico.'">
															<input type="hidden" name="codigoCerrarSesion" value="'.$codigoCerrarSesion.'">
															<input type="submit" class="submit" value="CERRAR SESION A TODOS LOS DISPOSITIVOS EN LINEA">
															</form>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div style="text-align:left">
                                                    <div style="font-family:Roboto-Regular,Helvetica,Arial,sans-serif;color:#b0b0b0;padding-top:10px;font-size:10px;line-height:18px">
                                                        <div>Te enviamos este correo electrónico para notificarte acerca de la seguridad de tu cuenta.</div>
                                                         <div style="font-family:Roboto-Regular,Helvetica,Arial,sans-serif;color:#b0b0b0;direction:ltr;font-size:10px;line-height:18px">© 2017 simpleteamgroup.com Inc., <a href="https://simpleteamgroup.com/" target="_blank">Contactanos</a></div>
                                                    </div>
...
<footer class="footer container-fluid bg-4 text-center">
    ...
    function destroySessionOnline($idUsuario){
        $query = $this->db->query('SELECT idSesion FROM sesiones WHERE idUsuario =  "'.$idUsuario.'" ');

        if($query->num_rows() > 0){

            foreach ($query->result_array() as $row){
                if(session_folder.$row->idSesion == ""){
